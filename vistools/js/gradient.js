...
//------------------------------------------------------------------------------
Gradient.prototype.sampleRgbaCss = function(loc)
{
  var color = this.sample(loc);
  return Gradient.colorToCssRgbaString(color);
};
...
{
  var r = Math.round(color.red * 255).toString(16);
  r = (r.length < 2) ? "0" + r : r;
  var g = Math.round(color.green * 255).toString(16);
  g = (g.length < 2) ? "0" + g : g;
  var b = Math.round(color.blue * 255).toString(16);
  b = (b.length < 2) ? "0" + b : b;
  if (color.alpha === 1.0)
    return "#" + r + g + b;
  else
  {
    var a = Math.round(color.alpha * 255).toString(16);
    a = (a.length < 2) ? "0" + a : a;
    return "#" + r + g + b + a;
  }
};
...
//------------------------------------------------------------------------------
Gradient.colorToCssRgbaString = function(color)
{
  return "rgba(" +
  ...
  for (i = 0; i < parts.length; i++)
  {
    ...
    if (subparts[0].indexOf("#") === 0)
    {
      ...
      var a = (subparts[0].length > 7) ?
        parseInt(subparts[0].substr(7,2), 16) / 255 : 1;
