...
class Surveyor(object):
    ...
    def __init__(self, dir_path=""):
        ...
        self.config = None
        self.demographics_path_or_list = None
    ...
    def __str__(self):
        ...
        """
        print("Survey for dir '%s'" % self.sim_dir)
...
Count', 'none()')\n"
        ...
        epilogue = [
            ...
            u"    print '\\nElapsed time ' + repr(script_end - script_start) + \
' seconds.'\n"
        ]
        with open(out_file_path, "w", newline="\r\n", encoding="utf-8") as f:
            ...
            f.write(u"k_name = '%s'\n" % survey.sim_dir)
            f.write(u"k_dir = '%s'\n" % survey.sim_dir)
            f.write(u"k_out_dir = '%s'\n" % survey.out_dir)
            if isinstance(survey.demographics_path_or_list, basestring):
                f.write(u"k_demographics_path_or_list = '%s'\n" %
...
and output files.")
    ...
    # Done
    # script_end = tm.time()
    # print "\nElapsed time " + repr(script_end - script_start) + " seconds."
