...
class PrepMaker(object):
    ...
    def emit(survey, nodes_type, out_file_path):
        ...
        preface = [
            ...
            u"# This script is meant to be run from within the target \
simulation directory.\n\n",
            u"from __future__ import print_function\n",
            u"import time as tm\n",
            u"from os import path, getcwd\n",
            u"from vis_tools import VisSet, SpatialBinary, SpatialReports\n\n",
            u"# Constants\n"
        ]
        constants = [
...
===============\n",
            ...
            u"    vis_set.set_products_directory(k_products_dir)\n",
        ]
...
output directory\n",
            u"    # looking for SpatialReport_*.bin files. Later we'll add \
this to the VisSet\n",
            u"    # object.\n",
            u"    reports = SpatialReports(k_spatial_report_dir)\n"
        ]
...
Vectors. Once\n",
            u"    # created, we manually add the new report to the reports \
object\n",
            u"    # def combine(a, b):\n",
            u"    #     return int(a * b)\n",
...
Infectious_Vectors.bin'),\n",
            ...
            u"    #     combine)\n",
            u"    # report_path = path.join(k_products_dir,\n",
            u"    #     'SpatialReport_Infectious_Vectors_Count.bin')\n",
            u"    # infected_vectors_count.write_binary(report_path)\n",
            u"    # reports.add(report_path)\n"
        ]
        spatial_reports = [
            u"\n    # Add spatial reports (also establishes timestep count)\n",
            u"    vis_set.add_spatial_reports(reports)\n"
        ]
...
Count', 'none()')\n"
        ...
        writeout = [
            ...
            u"\n"
            u"    # Emit VisSet\n",
            u"    vis_set.write_as('visset.json', k_sim_url_root)\n",
            u"\n"
        ]
        epilogue = [
            u"\n    # Done\n",
...
float(script_end - script_start))\n"
        ...
        with open(out_file_path, "w", newline="\r\n", encoding="utf-8") as f:
            ...
            f.write(u"k_name = path.basename(getcwd())\n")
            f.write(u"k_sim_dir = r'%s'\n" % survey.sim_dir)
            f.write(u"k_sim_output_dir = r'%s'\n" % survey.out_dir)
            f.write(u"k_products_dir = path.join(k_sim_dir, \
path.relpath(path.dirname(path.abspath(__file__)), getcwd()))\n")
            if isinstance(survey.demographics_path_or_list, basestring):
                ...
                f.write(u"]\n")
            f.write(u"k_sim_url_root = '/' + path.basename(getcwd()) + '/'\n")
            f.write(u"k_spatial_report_dir = k_sim_output_dir\n")
            if survey.asset_map_path is not None:
                f.write(u"k_asset_map_path = r'%s'\n" % survey.asset_map_path)
            if survey.sim_id is not None:
                f.write(u"k_sim_id = r'%s'\n" % survey.sim_id)
            ...
            if survey.sim_id is not None:
                f.write(u"    vis_set.set_sim_id(k_sim_id)\n")

            # Emit code to set the asset map if there is one
            if survey.asset_map_path is not None:
                f.write(u"    vis_set.set_asset_map_path(k_asset_map_path)\n")
            ...
            if isinstance(survey.demographics_path_or_list, basestring):
                f.write(u"    vis_set.set_demographics("
                        u"k_demographics_path_or_list)\n")
            else:
                f.write(u"    vis_set.set_demographics_files("
                        u"k_demographics_path_or_list)\n")
...
'%s')\n" % (u"green" if i == 0 else u"blue"))
            ...
            f.write(u"    # Emit URL\n")
            f.write(u"    print('When using the standalone Vis-Tools server, \
use the following URL:')\n")
            f.write(u"    visset_path = path.join(k_products_dir, \
'visset.json').replace('\\\\', '/')\n")
            f.write(u"    visset_path = visset_path.replace('./', \
k_sim_url_root)\n")
            f.write(u"    print('http://localhost:8000/vistools/\
